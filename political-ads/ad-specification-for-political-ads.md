...
* **Controller (if different from Sponsor)** If the Controlling Entity is a separate legal entity from the Sponsor, the label will also display "Controller: \[Entity Name]" in addition to the Sponsor's name. This adds more text to the label, which can increase the overlay height further.
